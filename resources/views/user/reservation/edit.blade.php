...
<div class="container mx-auto p-6">
    ...
    <h1 class="text-3xl font-extrabold text-center text-indigo-600 mb-8">Edit Reservation</h1>
    <form action="{{ route('profile.reservations.update', ['id' => $reservation->id]) }}" method="POST" class="bg-white shadow-lg rounded-lg p-6" id="reservation-form">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select id="course_id" name="course_id" class="block w-full mt-1 rounded border-gray-300" required>
                ...
                @foreach($courses as $course)
                    <option value="{{ $course->course_id }}" {{ $reservation->course_id == $course->course_id ? 'selected' : '' }}>
                        {{ $course->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input id="participant_name" name="participant_name" type="text" list="clients_list" class="block w-full mt-1 p-2 border rounded" placeholder="Zoek client..." value="{{ $reservation->participant->first_name }} {{ $reservation->participant->last_name }}" required>

            <!-- Verborgen input voor het client ID -->
            <input type="hidden" id="participant_id" name="participant_id" value="{{ $reservation->participant_id }}" required>
        ...
        <div class="mb-4">
            ...
            <input id="start_time" name="start_time" type="datetime-local" value="{{ \Carbon\Carbon::parse($reservation->start_time)->format('Y-m-d\TH:i') }}" required class="block mt-1 w-full rounded border-gray-300" />
        ...
        <div class="mb-4">
            ...
            <input id="end_time" name="end_time" type="datetime-local" value="{{ \Carbon\Carbon::parse($reservation->end_time)->format('Y-m-d\TH:i') }}" required class="block mt-1 w-full rounded border-gray-300" />
        ...
        <div class="mb-4">
            ...
            <select id="session_type" name="session_type" class="block w-full mt-1 rounded border-gray-300" required>
                ...
                <option value="online" {{ $reservation->session_type == 'online' ? 'selected' : '' }}>Online</option>
                <option value="physical" {{ $reservation->session_type == 'physical' ? 'selected' : '' }}>Fysiek</option>
            </select>
        ...
        <div class="mb-4">
            ...
            <select id="payment_status" name="payment_status" class="block w-full mt-1 rounded border-gray-300" required>
                <option value="pending" {{ $reservation->payment_status == 'pending' ? 'selected' : '' }}>In afwachting</option>
                <option value="paid" {{ $reservation->payment_status == 'paid' ? 'selected' : '' }}>Betaald</option>
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea id="comments" name="comments" class="block mt-1 w-full rounded border-gray-300" rows="4">{{ old('comments', $reservation->comments) }}</textarea>
        </div>

        <!-- Feedback -->
        <div class="mb-4">
            <x-input-label for="feedback" :value="__('Feedback')" />
            <textarea id="feedback" name="feedback" class="block mt-1 w-full rounded border-gray-300" rows="4">{{ old('feedback', $reservation->feedback) }}</textarea>
        </div>
